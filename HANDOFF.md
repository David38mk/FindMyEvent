...
**Done:** Time-scope pin rings + map legend (Phase 1 backlog): `TimeScope` enum + `endsAt`/`timeScope` on `MapPin` ([models.dart](app/lib/core/models.dart) — ≤36h daily / ≤32d multi-day / longer ongoing), event pins get a colored ring (red/blue/green) around the category-colored icon per the decided pin visuals, places stay ringless. Collapsible `_MapLegend` (bottom-left, collapsed by default) explains pin kinds, rings, and DB categories. l10n EN+MK. No schema change — `ends_at` already flowed through `map_events`. Analyze + tests green.
**Next:** Last Phase 1 backlog item: tile caching. Then Phase 1 exit review against PLAN.md. Name decision still open: Merak vs Vajb (§5.7). OSM places cleanup pass still pending (dupes flagged 08-19).
**Blocked:** -
