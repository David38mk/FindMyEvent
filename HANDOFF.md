...
**Next:** David: run `npx supabase db push` (RPC migration pending), then `flutter run --dart-define-from-file=dart_defines.json` on device — first real end-to-end test. Remaining Phase 1: pin clustering, time-scope ring visuals + map legend, 18+ gate, tile caching. Name decision: Merak vs Vajb (PLAN.md §5.7).
**Blocked:** -
